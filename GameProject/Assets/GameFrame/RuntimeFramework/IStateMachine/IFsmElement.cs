...
namespace Duskvern
{
    public abstract class FsmElement
    {
        ...
        public FsmElement Init(string name, IFsmNode firstNode, Dictionary<E_FsmState, IFsmNode> fsmElementDic, bool warrning = false)
        {
            ...
            {
                DebugLogger.LogError($"{Name} 初始化状态机失败", false);
                return this;
            }
            ...
            foreach (var node in m_fsmElementDic.Values)
            {
                if (node == null)
                {
                    DebugLogger.LogError($"{node.State} 节点为空", false);
                    return this;
                }
                if (m_fsmBlackboard != null && !FsmBlackboard.Equals(node.Blackboard))
                {
                    DebugLogger.LogError($"{Name} 状态机节点黑板不一致", false);
                    return this;
                }
        ...
        public void AddFsmNode(E_FsmState state, IFsmNode node)
        {
            ...
            {
               DebugLogger.LogWarning($"{Name} 添加 {state} 的状态机节点失败", false);
               return;
            }
        ...
        public void RemoveFsmNode(E_FsmState state)
        {
            if (m_fsmElementDic.TryGetValue(state, out var node))
            {
                ...
                {
                    DebugLogger.LogWarning($"移除的状态是当前状态机节点， 先退出当前状态机节点", false);
            ...
            DebugLogger.LogWarning($"{Name} 没有找到 {state} 的状态机节点", false);
        }
        ...
        public void BackFsmNode()
        {
            ...
            {
                DebugLogger.LogWarning($"{Name} 上一个状态机节点为空， 回退失败", false);
                return;
            }
        ...
        public void SwitchFsmNode(E_FsmState state)
        {
            ...
            {
                DebugLogger.LogWarning($"{Name} 当前状态机节点为空", false);
            }
            if (m_fsmElementDic.TryGetValue(state, out var targetNode))
            {
                ...
                if (m_Warrning) DebugLogger.LogInfo($"{Name} 切换到 {state} 状态", false);
                return;
            }
            DebugLogger.LogError($"{Name} 切换状态机失败：没有找到 {state} 的状态机节点", false);
        }
        ...
        public void OnExecute()
        {
            if (m_currentNode == null)
            {
                DebugLogger.LogError($"{Name} 当前状态机节点为空", false);
                return;
            }
